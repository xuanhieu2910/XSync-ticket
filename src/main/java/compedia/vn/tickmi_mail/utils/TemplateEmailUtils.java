...
public class TemplateEmailUtils {
    ...
    public static String TAG_IMAGE_TEMPLATE = "<img id=\"QR_HERE\" " +
    ...
    public static String replaceTemplateEmail (List<String> pathQrs) {
        ...
        for (int i = 0 ; i < pathQrs.size(); i++) {
            String replacePath = TAG_IMAGE_TEMPLATE.replace(TEMPLATE_REPLACE,pathQrs.get(i));
            result+= replacePath;
        }
        return result;
    }

    public static String replaceTag (String root, String oldStr, String newStr) {
        return root.replace(oldStr,newStr);
    }
}
